...
| 2-9 | 定性分析プロンプト設計（Q1〜Q5フレームワーク）| ⬜ | `claude_analyzer.py` に `analyze_qualitative()` メソッド追加。REQUIREMENTS.md §3.1 定性分析 参照 |
| 2-10 | 定性スコア（0〜10）+ 4段階ラベル生成実装 | ⬜ | Strong/Moderate/Weak/Unknown + 根拠コメント（1〜2文）|
| 2-11 | 週次レポートへの定性分析セクション追加 | ⬜ | Top5各社のQ1〜Q5評価表 + 総合定性スコアを `watch_list.md` に追記 |
...
```
✅ 完了       : 24項目（データ基盤7・日次シグナル12・通知3・自動実行2）
⬜ 未着手     : 16項目（統合スコアリング実装5・定性分析4・バリュエーション表示1・LINE翌朝通知1・将来拡張5）
