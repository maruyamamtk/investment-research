...
【フィルタ構成】
  ...
  2-C: 売上高成長フィルタ
       - 四半期: 直近3四半期連続プラス OR 最新25%以上（yoy）
       - 年次: 直近2期以上の年次売上高がプラス成長（Step1から移管）
  2-D: ROE > 15%（J-Quants 財務諸表から計算）
  2-E: FCFプラス継続（直近2期）
  2-F: ネット有利子負債/EBITDA ≤ 3.0x
  → スコアリングで上位20銘柄を選定（バリュエーション/PEG含む）
  → 分類: 全条件銘柄 / EPS条件銘柄
"""
...
DEFAULT_WEIGHTS = {
    ...
    "payout_ratio": 0.10,
    "valuation_peg": 0.15,  # PEG比率（PER ÷ 売上CAGR%）: 低いほど割安
}
...
# ============================================================
# 2-C補: 年次売上高成長フィルタ（Step1から移管・正の成長条件化）
# ============================================================

def filter_annual_netsales_growth(
    eps_series_map: dict[str, dict],
    min_growth: float = 0.0,
    min_periods: int = 2,
) -> set[str]:
    """
    年次売上高成長フィルタ（J-Quants annual.net_sales を使用）。

    直近 min_periods 期以上の年次売上高が前期比 min_growth 超を継続している
    銘柄コードを返す。旧 Step1「売上高成長率 ≥ -10%」を正の成長条件として
    Step2 に移管したもの。
    ...
    for ticker, series in eps_series_map.items():
        ...
        if len(growth_rates) < min_periods:
            continue

        # 直近 min_periods 期の成長率がすべて min_growth 超
        if all(g > min_growth for g in growth_rates[:min_periods]):
            passed.add(ticker)

    logger.debug(f"  年次売上高フィルタ通過: {len(passed)}件")
    return passed
...
def apply_step2_analysis(
    ...
    if has_jquants:
        ...
        netsales_quarterly_set = filter_netsales_quarterly(series_for_tickers, min_revenue_growth_latest)
        netsales_annual_set = filter_annual_netsales_growth(series_for_tickers)
        roe_jq_set = filter_roe(series_for_tickers, min_roe)

        all_cond_set = (
            eps_annual_set & eps_quarterly_set
            & netsales_quarterly_set & netsales_annual_set
            & roe_jq_set
        )
        eps_only_set = (eps_annual_set & eps_quarterly_set) - all_cond_set

        logger.info(
            f"  J-Quantsフィルタ結果: 年次EPS={len(eps_annual_set)}, 四半期EPS={len(eps_quarterly_set)}, "
            f"四半期売上={len(netsales_quarterly_set)}, 年次売上={len(netsales_annual_set)}, ROE={len(roe_jq_set)}"
        )
...
def _score_stocks(df: pd.DataFrame, weights: dict) -> pd.DataFrame:
    ...
    # PEG比率スコア: PER ÷ (売上CAGR% または 1%) — 低いほど割安で高スコア
    # PEG ≤ 0.5 → 10点, PEG ≥ 5.0 → 0点
    pe = pd.to_numeric(df.get("pe_ratio", pd.Series(dtype=float)), errors="coerce")
    cagr_pct = pd.to_numeric(df.get("revenue_cagr", pd.Series(dtype=float)), errors="coerce") * 100
    cagr_pct_safe = cagr_pct.clip(lower=1.0)
    peg = (pe / cagr_pct_safe).where(pe > 0, other=np.nan)
    df["peg_ratio"] = peg
    df["score_valuation_peg"] = _normalize(-peg.fillna(5.0), lo=-5.0, hi=-0.5)
    ...
    df["total_score"] = (
        ...
        + df["score_valuation_peg"].fillna(0) * weights.get("valuation_peg", 0)
    ) * 10
    return df
...
def format_step2_table(df: pd.DataFrame) -> str:
    ...
    if "PEG比率" in display.columns:
        display["PEG比率"] = display["PEG比率"].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
